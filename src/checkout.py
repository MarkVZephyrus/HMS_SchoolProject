...
def chkout(tk):
    def get_data():
        r = cr.customers[srno.get("1.0", "end-1c")]
        if r == -1:
            ...
            Label(tk, text="{}".format(locale.currency(netCost))).grid(
                row=n+1, column=1)
            Button(tk, text="Check Out", command=lambda: cr.check_out(
                srno.get("1.0", "end-1c"))).grid(row=n+2, column=3)
